...
- **The README carries a results table, and a test refuses a number in it that
  is not in a committed result file.** `docs/NUMBERS.md` is the ledger behind it:
  every headline number with the file it comes from, the full ablation table, and
  the mechanism behind each of its six zero rows.
  `report/generated/macros.tex` is generated by `experiments/results_to_tex.py`
  and tracked rather than gitignored, so a second test can assert the commit it
  names is the commit the results were measured at. Both nets were proven red on
  a deliberate fault and restored.
- **Every pass in `docs/PASSES.md` now cites a measured ablation delta**, which
  Section 12 has required since P0 and no phase could supply until there was a
  harness. Two of the eight ablatable passes have a nonzero row.
- **Two CI steps switch on**: `pytest slow cells` in the main pipeline and the
  `full-matrix` job in the nightly, both at the phase the Section 19.0 activation
  table names. The nightly runs the whole benchmark suite against its budget into
  a scratch directory, never over the committed results.
- **D-0040: seven tests were marked `slow` when the model suite landed and CI has
  never run one of them**, because the only step that runs slow tests is the one
