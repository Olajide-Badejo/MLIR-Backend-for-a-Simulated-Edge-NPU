...
`818475cb`, `GENERATOR_VERSION` 1.1.0. **217 cells**, on an Intel Core
...
commit between them is red. These numbers are `80ac24d`'s content, measured one
commit before it existed, at the tree where the compiler half of D-0052 lands and
tiling reaches the suite. **The wiring commit's own re-record, one before this
one, moved no counted field of the 175 pre-existing cells at all**, which is why
this page carries one provenance rather than two.
...
**1.14 seconds per cell is the measured figure that replaces Section 2's 15
...
**It was 1.17 seconds at P12 and 1.27 at P11**, and the per cell cost barely
moved while the suite grew by 42 cells, which is what a suite whose cost is per
cell rather than per run looks like. The P11 to P12 difference is the
