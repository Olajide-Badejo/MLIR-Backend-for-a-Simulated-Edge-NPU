...
`08e3b62b`, `GENERATOR_VERSION` 1.1.0. **217 cells**, on an Intel Core
i7-14700K under WSL2, serially, on an idle machine.

**The sha is the wiring commit's parent and that is not a slip.** The suite was
re-run at the tree that puts `-npu-assign-layout`, `-npu-tile-to-scratchpad` and
`-npu-double-buffer` into `-O2`, before that tree was committed, because the
cell count assertions and the results have to move in one commit or the commit
between them is red. These numbers are that commit's content, measured one
commit before it existed.
...
**Nothing in this branch touches the convolution kernel's loop nest and nothing
tiles at `-O2`, so the instruction shapes the kernel is handed are P12's to the
byte.** The spread between the two runs is the host, and it is the reason this
...
- **The tiling and double buffering rows are coupled and the row is honest about
  ...
  `resnet_block`, `inception_block` and three others, and since every one of
  those is declined either way the row is still zero. **The coupling is real and
  the zero is not evidence that it is not.**
